...
$SQL="SELECT * 
	  FROM Artwork,Group_Artwork 
	  Where Artwork.Type = Group_Artwork.Name";
...
echo "<table border = 0>";
echo "<tr font-size:18px;><td>Group Name</td><td>Title</td><td>Year</td><td>Price</td></td></tr>";

while ($row=mysqli_fetch_assoc($result))
{
	if($row['Year']==$info || $row['Price']==$info || $row['Type']==$info || $row['Name']==$info)
	{
